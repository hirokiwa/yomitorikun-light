...
const renderList = (messages: LocaleMessages, keys: string[]): string =>
  `<ol class="privacy-policy__list">${keys.map((key) =>
    `<li class="privacy-policy__list-item">${getMessage(messages, key)}</li>`).join('')}</ol>`;
...
<main class="main-contents">
  <article id="privacy-policy" aria-labelledby="page-title" class="privacy-policy__content">
    ...
    ${renderSection(messages, 4, renderList(messages, [
      'privacy.section4.body1',
      'privacy.section4.body2',
    ]))}
    ${renderSection(messages, 5, `
    ...
    ${renderSection(messages, 6, `${renderParagraphs(messages, ['privacy.section6.body1'])}${renderList(
      messages,
      ['privacy.section6.item1', 'privacy.section6.item2', 'privacy.section6.item3'],
    )}${renderParagraphs(messages, ['privacy.section6.body2'])}`)}
    ${renderSection(messages, 7, renderList(messages, [
      'privacy.section7.body1',
      'privacy.section7.body2',
      'privacy.section7.body3',
    ]))}
    ${renderSection(messages, 8, renderList(messages, [
      'privacy.section8.body1',
      'privacy.section8.body2',
      'privacy.section8.body3',
      'privacy.section8.body4',
    ]))}
    ${renderSection(messages, 9, renderList(messages, [
      'privacy.section9.body1',
      'privacy.section9.body2',
    ]))}
    ${renderSection(messages, 10, `
